...
rownames(info2018) = info2018$Country
hclusters <- hclust(dist(scale(info2018[, 4:7])))

pca_2018 = prcomp(info2018[,4:7], scale = TRUE)

scores = as.data.frame(predict(pca_2018, info2018[,4:7]))
clusters = kmeans(scores[,1], 3, nstart = 25)
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Country Data (2018) Clustering", tabName = "handkpca", icon = icon("th")),
      ...
      tabItem(tabName = "stats",
              fluidRow(box(width=7, status="warning",plotlyOutput("OvMap"), 
                       ...
                       box(width=5, title=strong("Input",style="font-family:'Verdana'"),status="info",solidHeader = TRUE,
                           ...
                           br(),
                           radioButtons("MaPcaT", label = "Select statistic:",
              ...
              tabItem(tabName = "measures",
                      ...
                      fluidRow(box(width = 4,status="info", title = strong("Input",style="font-family:'Verdana'"), solidHeader = T,
                                   h4("The right graph shows the ozone-depleting substances by country and year."),
                                   br(),
  ...
  server <- function(input, output) {
    ...
    output$Mgy = renderPlotly(ggplotly(ggplot() + geom_polygon(data=filter(CountrYrOz, Year == input$YeArrrrr), mapping = aes(long,lat,group=group, fill=Consumption, text=Country), size=0) +theme_void() +scale_fill_gradient(  low = "#99FFFF", na.value="#000000",high = "#FF0000")+theme(panel.background = element_rect(fill = '#FFFFFF', colour = 'blue'))))
    ...
    output$OvMap = renderPlotly(ggplotly(ggplot() + geom_polygon(data=filter(mapping, Year == input$YeAR2), mapping = aes_string("long","lat",group="group", fill=input$MaPcaT, text="Country"), size=0) +theme_void() +scale_fill_gradient(  low = "#99FFFF", na.value="#000000",high = "#FF0000")))
    ...
    output$pcaload = renderPlotly(ggplotly(ggbiplot(pca_2018,ellipse = TRUE, labels=rownames(pca_2018))))
    
    output$pcakmean = renderPlotly(ggplotly(ggplot(data = scores, aes(x = PC1, y = PC2,colour = clusters,text = country )) + geom_point()))
    ...
    output$hclus = renderPlot(plot(hclusters, hang = -1, cex = 1))
